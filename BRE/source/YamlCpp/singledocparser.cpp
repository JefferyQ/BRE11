...
namespace YAML
{
	...
	void SingleDocParser::HandleBlockSequence(EventHandler& eventHandler)
	{
		...
		for (;;) {
			...
			// check for null
			if (!m_scanner.empty()) {
				const Token& tempToken = m_scanner.peek();
				if (tempToken.type == Token::BLOCK_ENTRY || tempToken.type == Token::BLOCK_SEQ_END) {
					eventHandler.OnNull(tempToken.mark, NullAnchor);
					continue;
				}
			}
