#include "QuadCulledPointLightPsData.h"
...
namespace {
	const char* sShaderFile = "content\\shaders\\lightPasses\\fullyDeferred\\QuadCulledPointLightPS.cso";
}

namespace BRE {
	namespace FullyDeferred {
		QuadCulledPointLightPixelShaderData::QuadCulledPointLightPixelShaderData()
		...
		void QuadCulledPointLightPixelShaderData::InitializeCBuffers() {
		...
		void QuadCulledPointLightPixelShaderData::PreDraw(ID3D11Device1& device, ID3D11DeviceContext1& context, ID3D11ShaderResourceView* geometryBuffersSRVs[4]) {
			// Set shader
			ASSERT_PTR(mShader);
		...
		void QuadCulledPointLightPixelShaderData::PostDraw(ID3D11DeviceContext1& context) {
			// Set shader
