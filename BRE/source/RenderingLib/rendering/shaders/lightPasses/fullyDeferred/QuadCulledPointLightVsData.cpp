#include "QuadCulledPointLightVsData.h"
...
namespace {
	const char* sShaderFile = "content\\shaders\\lightPasses\\fullyDeferred\\QuadCulledPointLightVS.cso";
}

namespace BRE {
	namespace FullyDeferred {
		QuadCulledPointLightVertexShaderData::QuadCulledPointLightVertexShaderData()
		...
		DirectX::XMFLOAT4& QuadCulledPointLightVertexShaderData::LightPosAndRadius(const unsigned int index) {
			ASSERT_COND(index < sMaxLights);
			return mCBufferPerFrameData.mLightPosAndRadius[index];
		}

		DirectX::XMFLOAT4& QuadCulledPointLightVertexShaderData::LightColor(const unsigned int index) {
			ASSERT_COND(index < sMaxLights);
			return mCBufferPerFrameData.mLightColor[index];
		}

		void QuadCulledPointLightVertexShaderData::InitializeCBuffers() {
		...
		void QuadCulledPointLightVertexShaderData::PreDraw(ID3D11Device1& device, ID3D11DeviceContext1& context) {
		...
		void QuadCulledPointLightVertexShaderData::Draw(ID3D11DeviceContext1& context) {
			context.Draw(sMaxLights, 0);
		}

		void QuadCulledPointLightVertexShaderData::PostDraw(ID3D11DeviceContext1& context) {
			// Set shader
