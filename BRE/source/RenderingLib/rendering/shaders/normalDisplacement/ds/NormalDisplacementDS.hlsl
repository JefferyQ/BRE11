...
DS_OUTPUT main(const HS_CONSTANT_OUTPUT IN, const float3 uvw : SV_DomainLocation, const OutputPatch <HS_OUTPUT, NUM_PATCH_POINTS> patch) {
	...
	float4 posW = mul(uvw.x * patch[0].PosL + uvw.y * patch[1].PosL + uvw.z * patch[2].PosL, World);
	posW += float4(OUT.NormalW * (2 * DisplacementMap.SampleLevel(TexSampler, OUT.TexCoord, 0).x - 1) * DisplacementScale, 0.0f);
	OUT.PosH = mul(posW, ViewProj);
