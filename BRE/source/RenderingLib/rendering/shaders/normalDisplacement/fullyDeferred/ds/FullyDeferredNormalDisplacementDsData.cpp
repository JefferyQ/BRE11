#include "FullyDeferredNormalDisplacementDsData.h"
...
namespace {
	const char* sNormalDisplacementDS = "content\\shaders\\normalDisplacement\\fullyDeferred\\FullyDeferredNormalDisplacementDS.cso";
}

namespace BRE {
	namespace FullyDeferred {
		NormalDisplacementDsData::NormalDisplacementDsData()
			: mShader(ShadersManager::gInstance->LoadDomainShader(sNormalDisplacementDS))
		{
			ASSERT_PTR(mShader);
			InitializeCBuffers();
		}
